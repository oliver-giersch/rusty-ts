...
/** Method prototypes for the (union) {@link Result} type. */
abstract class ResultBase<T, E> {
    // can only be constructed by deriving classes
    protected constructor() { }

    /**
     * Returns `true` if the {@link Result} is an {@link Ok} variant.
     *
     * ## Example
     *
     * Using the `isOk` type guard narrow the {@link Result} allows the
     * contained {@link Ok} value to be accessed directly:
     *
     * ```typescript
    ...
    /**
     * Returns `true` if the {@link Result} is an {@link Err} variant.
     *
     * ## Example
     *
     * Using the `isErr` type guard narrow the {@link Result} allows the
     * contained {@link Err} value to be accessed directly:
     *
     * ```typescript
     * declare function getResult(): Result<number, Error>
     *
     ...
     * `ok` or `err` depending on the concrete variant held by `this`.
     *
     * ## Example
     *
     * ```typescript
    ...
    /**
     * Maps this {@link Result} to a `Result<U, E>` by applying `fn` to a
    ...
    /**
     * Maps this {@link Result} to a `U` instance by either calling `fn` on the
     * contained {@link Ok} value or returning the given default value.
     *
    ...
    /**
     * Maps this {@link Result} to a `U` by either calling `fn` on the
     * contained {@link Ok} value or returning the default value generated by
     * the given `def` function.
     *
     * The difference to {@link ResultBase.mapOr} is, that the default value is
    ...
    /**
     * Performs a logical *and* on `this` and `other`, returning `other` if the
     * former is {@link Ok} or otherwise the {@link Err} value of itself.
     *
    ...
    and<U>(this: Result<T, E>, other: Result<U, E>): Result<U, E> {
        switch (this.tag) {
            case Tag.Ok: return other
            case Tag.Err: return this as unknown as Result<U, E>
        }
    }

    /**
     * Performs a logical *and* on `this`, returning the {@link Result} returned
     * by `fn`, if the former is {@link Ok} or otherwise the {@link Err} value
     * of itself.
     *
     * @param fn the function generating the alternative {@link Result}
     * @returns the respective {@link Result}
     */
    andThen<U>(this: Result<T, E>, fn: (_: T) => Result<U, E>) {
        switch (this.tag) {
            case Tag.Ok: return fn(this.val)
            case Tag.Err: return this as Result<U, E>
        }
    }

    /**
     * Performs a logical *or* on `this`, returning the {@link Ok} value of
     * itself or otherwise `other`.
     *
     * @param other the other {@link Result} instance
     */
    or<F>(this: Result<T, E>, other: Result<T, F>): Result<T, F> {
        switch (this.tag) {
            case Tag.Ok: return this as unknown as Result<T, F>
            case Tag.Err: return other
        }
    }

    /**
     * Performs a logical *or* on `this`, returning the {@link Ok} value of
     * itself or otherwise returns the {@link Result} generated by `fn`.
     *
     * @param res the other {@link Result} instance
     */
    ...
    /**
     * Transforms `this` into a {@link Some} of its {@link Ok} variant and a
     * {@link None} otherwise.
     */
    ...
    /**
     * Transforms `this` into a {@link Some} of its {@link Err} variant and a
     * {@link None} otherwise.
     */
    ...
    /**
     * Returns the contained {@link Ok} value, if there is one, or throws an
     * {@link UnwrapError} exception otherwise.
     */
    ...
    /**
     * Returns the contained {@link Ok} value, if there is one, or the given
     * `def` value otherwise.
     *
    ...
    /**
     * Returns the contained {@link Ok} value, if there is one, or the value
     * generated by `def` otherwise.
     *
     * Unlike {@link ResultBase.unwrapOr}, the default value is hence lazily
     * generated only in case there is no contained {@link Ok} value.
     *
 ...
 *
 * The discriminant field (`tag`) is of type {@link Tag} and the {@link Ok}
 * variant includes an additional `val` field, whereas the {@link Err} variant
 * includes an `err` field.
 */
...
export const Result: {
    ...
    /**
     * Attempts to execute `fn`, wrapping its result in an {@link Ok} variant or
     * catches any exceptions that are instances of `err`, returning them as
     * {@link Err}.
     *
     * If an exception is caught that is not an instance of `err`, it will be
     * re-thrown.
     *
     * @param fn the function to be executed
     * @param err the exception to be caught
     * @returns the {@link Result} wrapping either `fn`'s return value or the
     *   caught error
...
/**
 * The error that is thrown when calling `unwrap[_err]`
 * on an instance containing the wrong variant.
 */
export class UnwrapError<T> extends Error {
    /**
     * Error constructor
     *
     * @param val the actually contained (unexpected) value
     * @param expected the expected tag value
     */
...
/** The error that is thrown when calling `expect` on an {@link Err} variant. */
export class ExpectError<E> extends Error {
    /**
     * Error constructor
     *
     * @param err the (unexpected) contained error
     * @param msg the custom error message
     */
    constructor(public err: E, msg: string) {
        super(msg + ': ' + err)
    }
}